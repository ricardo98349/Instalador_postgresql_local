...
get_max_whats() {
  ...
  print_banner
  printf "${WHITE} 💻 Informe a Qtde de Conexões/Whats que a ${instancia_add} poderá cadastrar (Exemplo: 10000):${GRAY_LIGHT}"
  printf "\n\n"
  read -p "> " max_whats
}
...
get_max_user() {
  ...
  print_banner
  printf "${WHITE} 💻 Informe a Qtde de Usuarios/Atendentes que a ${instancia_add} poderá cadastrar (Exemplo: 10000):${GRAY_LIGHT}"
  printf "\n\n"
  read -p "> " max_user
}
...
get_backend_port() {
  ...
  print_banner
  printf "${WHITE} 💻 Digite a porta do BACKEND para esta instancia; Ex: 8081 ${GRAY_LIGHT}"
  printf "\n\n"
  read -p "> " backend_port
}
