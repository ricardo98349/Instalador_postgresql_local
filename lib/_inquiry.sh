...
get_instancia_add() {
  ...
  print_banner
  printf "${WHITE} 💻 Informe um nome para a Instancia/Empresa que será instalada ( Exemplo: empre Não usar letras Maiusculas e caracteres especiais ):${GRAY_LIGHT}"
  printf "\n\n"
...
get_max_whats() {
  ...
  print_banner
  printf "${WHITE} 💻 Informe a Qtde de Conexões/Whats que a ${instancia_add} poderá cadastrar (Exemplo: 1000):${GRAY_LIGHT}"
  printf "\n\n"
  read -p "> " max_whats
}
...
get_max_user() {
  ...
  print_banner
  printf "${WHITE} 💻 Informe a Qtde de Usuarios/Atendentes que a ${instancia_add} poderá cadastrar (Exemplo: 1000):${GRAY_LIGHT}"
  printf "\n\n"
  read -p "> " max_user
}
...
get_redis_port() {
  ...
  print_banner
  printf "${WHITE} 💻 Digite a porta do REDIS/AGENDAMENTO MSG para a ${instancia_add}; Ex: 5001 ${GRAY_LIGHT}"
  printf "\n\n"
  read -p "> " redis_port
}
...
inquiry_options() {
  ...
  printf "   [1] Atualizar Whamulti\n"
  printf "   [2] Deletar Whamulti\n"
  printf "   [3] Bloquear Whamulti\n"
  printf "   [4] Desbloquear Whamulti\n"
  printf "   [5] Alter. dominio Whamulti\n"
  printf "\n"
  read -p "> " option
